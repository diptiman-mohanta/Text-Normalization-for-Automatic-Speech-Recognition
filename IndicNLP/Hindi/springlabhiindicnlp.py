...
class HindiASREvaluator:
    def __init__(self, model_path: str, dict_path: str = None, use_cpu: bool = False):
        """
        Initialize the ASR evaluator with model path.
        Uses fairseq's checkpoint loading method for SPRING-INX models.
        
        Args:
            model_path: Path to SPRING_INX_wav2vec2_Hindi.pt
            dict_path: Path to SPRING_INX_Hindi_dict.txt (optional)
        ...
        self.model, self.cfg, self.task = self.load_model(model_path, dict_path)
        ...
        # Initialize Indic NLP normalizer for Hindi
        print("Initializing Indic NLP normalizer for Hindi...")
        self.indic_normalizer = IndicNormalizerFactory().get_normalizer("hi")
        print("Indic NLP normalizer initialized")
        
    def load_model(self, model_path: str, dict_path: str = None):
        """Load the fairseq model using checkpoint_utils."""
        # Prepare arg_overrides
        arg_overrides = {"data": "/"}
        
        # If dictionary path is provided, add it to overrides
        if dict_path:
            arg_overrides["dict"] = dict_path
    ...
    def is_devanagari_char(self, char: str) -> bool:
        """Check if character is in Devanagari Unicode range (Hindi)."""
        if not char:
            return False
        return '\u0900' <= char[0] <= '\u097F'
    
    def join_devanagari_characters(self, text: str) -> str:
        """
        Join separated Devanagari (Hindi) characters back into complete words.
        Uses | as primary word boundary, with fallback heuristics.
        
        Args:
            text: Space-separated Devanagari characters with | as word boundaries
            
        Returns:
            Properly joined Hindi text
        """
        ...
        dependent_vowels = set(['ा', 'ि', 'ी', 'ु', 'ू', 'ृ', 'े', 'ै', 'ो', 'ौ', 'ं', 'ः', 'ँ'])
        
        # Virama (halant) - combining mark
        virama = '्'
        ...
        for i, char in enumerate(chars):
            ...
            # Handle Devanagari characters
            if self.is_devanagari_char(char):
                # Always join dependent vowels and combining marks
                if char in dependent_vowels:
                    current_word.append(char)
                # If previous character was virama (halant), join this character
                elif current_word and current_word[-1] == virama:
                    current_word.append(char)
                # Otherwise, add to current word (we'll rely on | for word boundaries)
                else:
                    current_word.append(char)
            else:
                # Non-Devanagari character, treat as word boundary
                if current_word:
                    result.append(''.join(current_word))
                    current_word = []
                if char.strip():
    ...
    def transcribe(self, audio_path: str) -> str:
        ...
        try:
            ...
            # Join Devanagari characters properly
            transcription = self.join_devanagari_characters(transcription)
    ...
    def normalize_text_basic(self, text: str) -> str:
        """Basic normalization for Hindi text (used for raw metrics)."""
        # Normalize Unicode (NFC normalization for Devanagari)
        text = unicodedata.normalize('NFC', text)
        
        # Remove punctuation (question mark, comma, full stop, and Devanagari danda)
        text = re.sub(r'[?,।.!;:()"\'\[\]{}]', '', text)
    ...
    def normalize_text_indic(self, text: str) -> str:
        """
        Normalize Hindi text using Indic NLP Library with additional normalizations.
        ...
        # Additional Hindi-specific normalizations
        # Normalize nukta variants
        text = text.replace('क़', 'क')
        text = text.replace('ख़', 'ख')
        text = text.replace('ग़', 'ग')
        text = text.replace('ज़', 'ज')
        text = text.replace('ड़', 'ड')
        text = text.replace('ढ़', 'ढ')
        text = text.replace('फ़', 'फ')
        text = text.replace('य़', 'य')
        
        # Normalize chandrabindu and anusvara variations
        # (keeping them as is since they have different meanings)
        
        # Remove extra whitespace and normalize spacing
        text = re.sub(r'\s+', ' ', text)
        text = text.strip()
        
        # Convert to lowercase (if applicable for English/digits)
        text = text.lower()
        
        return text
    
    def evaluate_dataset(self, json_path: str, output_txt: str = "evaluation_results_hindi.txt", output_csv: str = "evaluation_results_hindi.csv"):
        """
        ...
        for idx, item in enumerate(dataset):
            ...
            # Apply basic normalization for raw metrics (only Unicode normalization + punctuation removal)
        ...
        with open(output_txt, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("HINDI ASR EVALUATION RESULTS (SPRING-INX Model with Indic NLP)\n")
            f.write("=" * 80 + "\n\n")
...
def main():
    ...
    MODEL_PATH = "D:\\SPIRE Internship\\SPRING_INX_wav2vec2_Hindi.pt"
    DICT_PATH = "D:\\SPIRE Internship\\SPRING_INX_Hindi_dict.txt"  # Optional, can be None
    JSON_PATH = "D:\\SPIRE Internship\\dataset_all.json"
    OUTPUT_TXT = "evaluation_results_hindi_indic.txt"
    OUTPUT_CSV = "evaluation_results_hindi_indic.csv"
    ...
    print("Initializing Hindi ASR Evaluator with Indic NLP...")
    evaluator = HindiASREvaluator(MODEL_PATH, dict_path=DICT_PATH, use_cpu=USE_CPU)
